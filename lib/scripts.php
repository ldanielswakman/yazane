...
/**
 * Enqueue custom theme style for admin screens
 */
function enqueue_admin_theme_style() {
  $uri = get_template_directory_uri();
  if (WP_ENV === 'development') {
    $uri .= '/assets/css/wp-admin.css';
  } else {
    $get_assets = file_get_contents(get_template_directory() . '/assets/manifest.json');
    $assets     = json_decode($get_assets, true);
    $uri .= '/assets/css/wp-admin.min.css';
    $uri .= $assets['assets/css/wp-admin.min.css']['hash'];
  }

  wp_enqueue_style('yazane_admin_css', $uri, false, null);
}
add_action('admin_enqueue_scripts', 'enqueue_admin_theme_style');
add_action('login_enqueue_scripts', 'enqueue_admin_theme_style');
