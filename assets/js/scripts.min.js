...
$(document).ready(function() {
  ...
  if ($('#map-canvas').length > 0) {
    var mapOptions = {
      ...
      center: { lat: 41.032345, lng: 28.976031 },
      zoom: 15,
      scrollwheel: false,
  ...
  // Social share button interaction
  function launchShareDialog(i,n,g,k) {
  ...
  $('.btn-share-twitter').on('click', function(e) {
    launchShareDialog("https://twitter.com/share?url=" + document.location.href + "&text=" + document.title + "&via=Yazanecowork","twitter-share-dialog",500,300);
  });
  $('.btn-share-facebook').on('click', function(e) {
    launchShareDialog("https://www.facebook.com/sharer/sharer.php?u=" + document.location.href,"facebook-share-dialog",500,400);
  });
  $('.btn-share-linkedin').on('click', function(e) {
    launchShareDialog("http://www.linkedin.com/shareArticle?mini=true&url=" + document.location.href + "&title=" + document.title + "&summary=" + $("head meta[name=description]").attr("content") + "&source=Yazane","linkedin-share-dialog",400,400);
  });
  ...
  if ($('body').hasClass('page') && $('body').find('.coworker-container').length > 0) {
    ...
    $('.coworker-sorting a').bind(touchEvent, (function(e) {
      // setting Isotope sorting
      $('.coworker-container').isotope({
        sortBy: $(this).attr('data-sort')
      });
      // toggling button states
      $('.coworker-sorting a').removeClass('btn-primary');
      $(this).addClass('btn-primary');
    }));
    // Isotope: coworker filtering buttons
    $('.coworker-filtering a').bind(touchEvent, (function(e) {
      // setting Isotope sorting
      buttonFilter = '.' + $(this).attr('data-filter');
      $('.coworker-container').isotope();
      // toggling button states
      $('.coworker-filtering a').removeClass('btn-primary');
      $(this).addClass('btn-primary');
    }));
    // Isotope: coworker search
    var $quicksearch = $('#coworkersearch').bind('keyup change', (debounce( function() {
    ...
    $('#coworkersearch_clear').bind(touchEvent, (function(e) {
      $('#coworkersearch').val('');
      qsRegex = new RegExp( $quicksearch.val(), 'gi' );
      $('.coworker-container').isotope();
      $quicksearch.removeClass('hasContent');
    ...
    $('.coworker').bind('click', (function(e) {
      ...
      for (i=0; i<$data.length; i++) {
        ...
        $data_item = ($coworker_data_item) ? $coworker_data_item : '–';
        $('dialog#coworker_detail').find('.coworker-'+$data[i]).html($data_item).scrollTop(0);
      }
    ...
    $('.company').bind('click', (function(e) {
      ...
      for (i=0; i<$data.length; i++) {
        ...
        $data_item = ($company_data_item) ? $company_data_item : '–';
  ...
  if ($('body').hasClass('page') && $('body').find('#calendars .calendar').length > 0 && $('body').find('.btn-tab').length > 0) {
    var calendars = ["#event_space_1", "#event_space_2", "#meeting_room_1", "#meeting_room_2"];

    var showCalendar = function(calendar) {
      if (calendar == '') {
        $('.btn-tab').removeClass("current");
        $(".tabs-group .btn-tab").first().addClass("current");
        $('section#calendars .calendar').hide();
        $('section#calendars .calendar').first().show();
      } else if ($.inArray(calendar, calendars) != -1) {
        $('.btn-tab').removeClass("current");
